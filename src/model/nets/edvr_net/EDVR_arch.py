...
import functools
import src.model.nets.edvr_net.arch_util as arch_util
try:
    from src.model.nets.edvr_net.dcn import ModulatedDeformConvPack as DCN
except ImportError:
    raise ImportError('Failed to import DCNv2 module.')
from src.model.nets.base_net import BaseNet


class EDVRNet(BaseNet):
    """The implementation of the EDVR.

    Refs: https://arxiv.org/abs/1905.02716,
          https://github.com/open-mmlab/mmsr.
    """
    def __init__(self, in_channels, out_channels, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10,
        ...
        if self.is_predeblur:
            self.pre_deblur = Predeblur_ResNet_Pyramid(in_channels, nf=nf, HR_in=self.HR_in)
            self.conv_1x1 = nn.Conv2d(nf, nf, 1, 1, bias=True)
        else:
            if self.HR_in:
                self.conv_first_1 = nn.Conv2d(in_channels, nf, 3, 1, 1, bias=True)
        ...
        self.conv_last = nn.Conv2d(64, out_channels, 3, 1, 1, bias=True)
    ...
    def forward(self, inputs):
        ...
        if H % 4 != 0 or W % 4 != 0:
            has_padded = True
            H_diff = 4 - H % 4 if H % 4 != 0 else 0
            W_diff = 4 - W % 4 if W % 4 != 0 else 0
        ...
        if has_padded:
            W0, Wn, H0, Hn = pad
            W0, Wn, H0, Hn = W0 * 4, out.size(-1) - Wn * 4, H0 * 4, out.size(-2) - Hn * 4
            out = out[..., H0:Hn, W0:Wn]
        return out


class Predeblur_ResNet_Pyramid(nn.Module):
    def __init__(self, in_channels, nf=128, HR_in=False):
        super().__init__()
        self.HR_in = True if HR_in else False
        if self.HR_in:
            self.conv_first_1 = nn.Conv2d(in_channels, nf, 3, 1, 1, bias=True)
