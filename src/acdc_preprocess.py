...
def main(args):
    # Random split the data for training and validation (8:2).
    paths = sorted((args.data_dir / 'training').glob('**/*4d.nii.gz'))
    random.seed('vsr') # Use the defined random seed.
    ...
    for type_, paths in zip(['train', 'valid', 'test'], [train_paths, valid_paths, test_paths]):
        ...
        for path in paths:
            ...
            data = nib.load(str(path)).get_data() # (H, W, D, T)
            ...
            for i, downscale_factor in enumerate([2, 3, 4]):
                ...
                for s in range(data.shape[2]):
                    ...
                    nib.save(nib.Nifti1Image(lr_video, np.eye(4)),
                             str(lr_videos_dir / f'{patient_name}_2d+1d_sequence{s+1:0>2d}.nii.gz'))
                    for t in range(data.shape[-1]):
                        if i == 0:
                            hr_img = hr_video[..., t] # (H, W, C)
                            nib.save(nib.Nifti1Image(hr_img, np.eye(4)),
                                     str(hr_imgs_dir / f'{patient_name}_2d_slice{s+1:0>2d}_frame{t+1:0>2d}.nii.gz'))
