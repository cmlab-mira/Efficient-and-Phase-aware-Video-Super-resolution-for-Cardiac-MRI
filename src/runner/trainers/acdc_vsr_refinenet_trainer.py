...
class AcdcVSRRefineNetTrainer(BaseTrainer):
    ...
    def _run_epoch(self, mode):
        ...
        for batch in trange:
            ...
            inputs, targets, pos_codes = self._get_inputs_targets(batch)
            T = len(inputs)
            if mode == 'training':
                outputs = self.net(inputs, pos_codes)
    ...
    def _compute_losses(self, outputs, targets):
        ...
        if self.net.training:
            ...
            for loss_fn in self.loss_fns:
                loss = []
                for i, _outputs in enumerate(outputs):
                    discount_factor = np.power(0.5, (len(outputs) // 3 - i // 3 - 1))
                    # Average the losses computed at every time steps.
                    loss.append(torch.stack([loss_fn(output, target) * discount_factor 
                                             for output, target in zip(_outputs, targets)]).mean())
